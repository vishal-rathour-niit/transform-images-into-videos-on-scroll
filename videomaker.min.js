/* 
    lib name: transform images into videos on scroll,
			...
			createPreImage: async function() {
				...
				l.forEach(function(t, a) {
					...
					f.loadImage(r, n).then(function() {
						0 === m && (f.event('start'), onMethod['re-loading'] && onMethod['re-loading'].call(this,{'status':'start','view':resizeManage.isDesktop ? 'desktopView':'mobileView'})), 
						m += 1, 
						percentage = Math.floor((m / u) * 100),
						onMethod['percentageCompleted'] && onMethod['percentageCompleted'].call(this,percentage),
						u === m && (f.event('end') , onMethod['re-loading'] && onMethod['re-loading'].call(this,{'status':'end','view':resizeManage.isDesktop ? 'desktopView':'mobileView'}))
					});
			...
			settelCanvasImage: function(e) {
				...
				if(autoRatio){
					t.addEventListener("load", function() {
						s.clearRect(0, 0, window.innerWidth, window.innerHeight);
						s.canvas.width = window.innerWidth;
						s.canvas.height = window.innerHeight;
						const imgAspectRatio = t.width / t.height;
						const canvasAspectRatio = s.canvas.width / s.canvas.height;
						let srcX, srcY, srcWidth, srcHeight;
						if (imgAspectRatio > canvasAspectRatio) {
							srcHeight = t.height;
							srcWidth = srcHeight * canvasAspectRatio;
							srcX = (t.width - srcWidth) / 2;
							srcY = 0;
						} else {
							srcWidth = t.width;
							srcHeight = srcWidth / canvasAspectRatio;
							srcX = 0;
							srcY = (t.height - srcHeight) / 2;
						}
						s.drawImage(t, srcX, srcY, srcWidth, srcHeight, 0, 0, s.canvas.width, s.canvas.height);
					}, !1), t.src = e;
				}
				else{
					t.addEventListener("load", function() {
						const height = t.width * 9 / 16;
